...
> 研究與建檔日期：2026-09-02
>
> 狀態：已完成第 1 批（5 篇專題發布），其餘主題已分批結構化歸檔，供後續排期撰寫。
...
| **第 2 批** | 影音、社交與即時高併發管線（Twitter / YouTube / TikTok / Uber）        | 4 篇     | 0 篇       | ⏳ 排入待辦佇列   |
| **第 3 批** | 現代通訊協定、API 閘道與網路流量工程（HTTP/3 / gRPC / 閘道邊界）       | 4 篇     | 0 篇       | 📋 儲備中         |
...
| :------------------------------------------- | :----------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------- |
| **Twitter (X) 1.5 秒渲染「為你推薦」時間軸** | Earlybird 向量檢索、Heavy Ranker 評分、Home Mixer 推薦管線、GraphJet           | [Twitter: The Algorithm & Engineering Blog](https://blog.x.com/engineering/en_us/topics/open-source/2023/twitter-recommendation-algorithm) |
| **YouTube 海量影片非同步上傳與分塊轉碼串流** | Chunked Upload、非同步 DAG 工作流、自適應碼率（HLS / MPEG-DASH）、CDN 邊緣快取 | [Google Cloud / YouTube Video Architecture](https://cloud.google.com/solutions/media-entertainment)                                        |
| **TikTok 20 萬檔案超大型前端 MonoRepo 治理** | 模組邊界隔離、增量建置快取、CI 平行化調度、微前端架構                          | [ByteDance Engineering Blog](https://bytedance.feishu.cn/)                                                                                 |
| **Uber 全球 API 閘道架構演進**               | 從單體 Edge 到分散式 Gateway、gRPC 反向代理、動態負載均衡與多區域流量切換      | [Uber: Evolution of Uber's API Layer](https://www.uber.com/blog/evolution-of-ubers-api-layer/)                                             |
