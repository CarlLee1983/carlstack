...
| Markdown 管線    | `astro.config.mjs` 的 `markdown.processor`、`src/utils/*.mjs` 外掛                             | 外掛順序有依賴：`rehype-heading-anchor` 需要 `rehypeHeadingIds` 先產生 id；`rehype-table-scroll` 為表格單元格提取注入 `data-label` 並包裹捲動容器，支援行動端轉卡片排版與桌機水平捲動                                        |
...
| 主題解析         | `src/layouts/BaseLayout.astro` 的 inline script                                                | `data-theme` 一律是解析後的 `light`／`dark`，localStorage 只存使用者的明確選擇。元件只判斷 `:root[data-theme="dark"]`，不自行處理 `prefers-color-scheme`（[ADR 0005](adr/0005-diagram-dark-mode-selector.md)）               |
