...
- Markdown 表格只用於真正的欄列資料（建議 2~~4 欄），適用於短詞、數值與參數對照；禁止將包含多個長句子描述（如案例情境、模型判斷、接續動作與限制邊界）的多維度目錄硬塞進 4 欄以上的寬表格。遇到多屬性案例或架構圖譜，應使用「2~~3 欄高層索引表 ＋ 結構化模組卡片（h3/h4 搭配鍵值清單）」排版，兼顧閱讀節奏與全螢幕適配。
