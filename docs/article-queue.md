...
- [ ] **第 2 篇（撰文中）：** 《別讓 Agent 偽造世界：如何用 Seam 與 In-Memory Fake 終結 AI 的 Mock 地獄？》（Michael Feathers 接縫哲學、Replace-don't-layer、雙 Adapter 準則、In-Memory Fake 取代脆性 Mock）
- [ ] **第 3 篇（排隊中）：** 《消滅 Agent 的語義漂移：從 `CONTEXT.md` 到防禦型規格工作流》（DDD 通用語言、純度控制、Sharpen Fuzzy Language 追問防線、輕量化 ADR 準則）
- [ ] **第 4 篇（排隊中）：** 《給 Agent 一把手術刀：垂直切片（Tracer Bullets）與小表面積測試》（打破三層架構跳檔困局、單一完整功能路徑、Interface as Test Surface、無副作用純淨介面）
