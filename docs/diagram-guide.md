...
3. **字級與可讀性下限（桌面端 viewBox 座標系）**：
   ...
   - 階段 / 節點標籤：`13px` ~ `15px`（`font-weight: 700`）。
   - 內文說明 / 註解：`12.5px` ~ `13.5px`（`fill: var(--dg-ink-2)`）。
   - **硬性禁止使用小於 12px 的說明文字（小標籤/Badge 極限不得低於 11px）**；桌面端文字若低於此下限，縮放至常規筆電與平板時會嚴重模糊。
4. **單一圖解職責與複雜度解耦**：
...
7. **正文與圖解的排版互補原則（Prose-Diagram Synergy）**：
   - 正文在引用圖解時，**禁止以冗長凌亂的條列純文字重述圖解細節**；
   - 應使用**精確對齊的 Markdown 表格（路徑 / 節點 / 預存結果 / 複雜度對比）**或醒目的架構 Callout，形成「圖解直觀建立心智模型 ➔ 下方表格精準提供工程參數」的高質感閱讀體驗。
