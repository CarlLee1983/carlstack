...
3. **字級與可讀性下限（桌面端 viewBox 座標系）**：
   - 主標題 / 核心卡片標題：`16px` ~ `19px`（`font-weight: 800`）。
   - 階段 / 節點標籤 / 子卡標題：`13.5px` ~ `15px`（`font-weight: 700`）。
   - 核心摘要導引句（Lead text）：`14px` ~ `14.5px`（`font-weight: 700`，`fill: var(--dg-ink)`）。
   - 內文說明 / 註解：`13px` ~ `13.5px`（`fill: var(--dg-ink-2)`）。
   - **硬性禁止使用小於 12.5px 的說明文字（小標籤/Badge 極限不得低於 11px）**；桌面端文字若低於此下限，縮放至常規筆電與平板時會嚴重模糊。
4. **單一圖解職責與複雜度解耦**：
...
6. **樹狀階層與多層級架構的「寬卡片分列佈局」（Row Card Pattern）**：
   - 當展示樹狀結構（如 Trie、B+ Tree）或層級調度（如分級時間輪）時，**嚴禁將深層節點文字全部塞進窄小垂直框線內**。
   - 採用**「左側圓形節點/前綴標籤 + 右側結構化預存清單卡片」**的整列寬幅佈局（橫向佔滿 860px 以上），確保右側結果方塊具備獨立的清單條目（List Bar），字級達到 `12px~13px`。
7. **嚴禁「四象限擁擠硬塞」（Anti-Quadrant Grid Cramming）**：
   - 當概念涉及「雙維度矩陣」（如：商業差異化 vs 業務複雜度、優先級 vs 成本）時，**嚴禁硬畫直角座標軸並把四個資訊卡強行塞在 2×2 的小方格中**。這會導致 X/Y 軸標籤貼邊無處安放、卡片內欄位破碎折行、字級暴跌至 11px 以下。
   - **標準解法：解構為「頂部維度橫幅導航 + 獨立全幅縱列（Three/Four-Column Layout）+ 底部通欄警示/反模式卡片」**。將 viewBox 高度拉開至 `540px ~ 600px`，讓每個分類都有獨立的寬卡片與呼吸空間。
8. **正文與圖解的排版互補原則（Prose-Diagram Synergy）**：
   - 正文在引用圖解時，**禁止以冗長凌亂的條列純文字重述圖解細節**；
   - 應使用**精確對齊的結構化卡片或粗體鍵值清單（路徑 / 節點 / 預存結果 / 複雜度對比）**或醒目的架構 Callout，形成「圖解直觀建立心智模型 ➔ 下方文字精準提供工程參數」的高質感閱讀體驗。
