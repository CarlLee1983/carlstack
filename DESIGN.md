...
- **表格 (`table`)**：
  - **大螢幕（> 40rem）**：卡片式獨立包覆、圓角外框、深淺自適應表頭、寬鬆內距（`1rem 1.25rem`）、容器水平捲動保護。
  - **行動端（<= 40rem）**：解構為自適應鍵值卡片。`thead` 採可訪問性隱藏，每一列 `tr` 轉為獨立卡片，單元格 `td` 透過 Rehype 注入之 `data-label` 生成標籤並以雙欄 Grid 鍵值排列，避免中文字元窄欄擠壓或強迫橫向捲動。
