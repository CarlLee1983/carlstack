...
1. 若 D == 0 (完美等間隔)：
   ➔ 僅寫入 1 個 bit: '0'
2. 若 -63 <= D <= 64：
   ➔ 寫入 '10' (2 bits) + 7 bits 值 (共 9 bits)
3. 若 -255 <= D <= 256：
   ➔ 寫入 '110' (3 bits) + 9 bits 值 (共 12 bits)
4. 若 -2047 <= D <= 2048：
   ➔ 寫入 '1110' (4 bits) + 12 bits 值 (共 16 bits)
5. 其餘情況：
   ➔ 寫入 '1111' (4 bits) + 32 bits 值 (共 36 bits)
```
...
```text
Gorilla Float64 浮點數壓縮流程：
1. 若 X == 0 (數值無變化)：
   ➔ 僅儲存 1 個 bit: '0'
2. 若 X != 0：
   ➔ 寫入 1 個 bit: '1'
   a. 若當前 XOR 結果的前導零 (Leading Zeros) 與尾隨零 (Trailing Zeros)
      落在前一個 XOR 結果的區間內：
      ➔ 寫入 '0' (控制位) + 直接儲存變化區間內的有效位元 (Meaningful bits)
   b. 若前導/尾隨零超出前一次區間：
      ➔ 寫入 '1' (控制位) + 5 bits (前導零個數) + 6 bits (有效位長度) + 有效位元
```
