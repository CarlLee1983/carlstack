...
- 當前綴節點為 `'s'` 時，其節點內部直接預存了全站以 `s` 開頭的最熱門前 3 筆結果（如 `spotify`, `speed test`, `steam`）；
- 當使用者鍵入第二個字母抵達 `'sy'` 節點時，該節點同樣早已計算好 `system design`、`synonym` 等權重結果；
- **查詢複雜度徹底降為 `O(p)`**！因為不需要任何額外的子節點遍歷或即時排序，調度器只需沿著前綴字元走 `p` 步，直接讀取目標節點所儲存的清單即可。
...
- 假設全站活躍且有意義的搜尋詞庫共有 **1,000 萬筆（10M）**；
- 英文單字平均長度為 6 個字元，Trie 樹大約會產生 3,000 萬個節點；
- 若每個節點儲存 Top 5 推薦詞的 ID 或指標，每個節點約額外消耗 `5 × 8 Bytes = 40 Bytes`；
- 加上 Trie 節點指針與邊元資料（約 64 Bytes），單個節點總計約 104 Bytes；
- **總記憶體需求**：
  `30,000,000 × 104 Bytes ≈ 3.12 GB`

**3.12 GB 的記憶體開銷，即便是單台平價伺服器都能輕易常駐於 RAM 中！** 以微不足道的 3GB 記憶體換取百萬 QPS 下小於 1ms 的純記憶體指標讀取，是系統架構中最划算的權衡之一。
