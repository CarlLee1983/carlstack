...
```text
               [ C ] Consistency (線性一致性 / 強一致性)
                    /   \
                   /     \
                  /  CAP  \
                 /  Trade- \
                /    off    \
[ A ] Availability ───────── [ P ] Partition Tolerance (分區容忍性)
```
...
[ Client Request: Set x=5 ]
           │
           ▼
[ Leader Log: Index 1, Term 1, Set x=5 (Uncommitted) ]
     ├── AppendEntries RPC ──> [ Follower A Log (寫入本地磁碟) ]
     └── AppendEntries RPC ──> [ Follower B Log (寫入本地磁碟) ]
           │
           ▼ (當過半數節點成功寫入日誌)
[ Leader Commit Index 推進 ! ] ── (應用至狀態機並回覆 Client 成功)
           │
           ▼ (下次心跳通知 Follower 將該日誌提交並應用至狀態機)
```
