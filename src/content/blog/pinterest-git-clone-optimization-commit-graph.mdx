...
```
[ Commit Object ] ───► [ Tree Object (目錄) ] ───► [ Blob Object (檔案內容) ]
       │
       ▼ (Parent 指標)
[ Commit Object ] ───► [ Tree Object (目錄) ] ───► [ Blob Object (檔案內容) ]
```
...
```
Server (Wants: C_500000)
    │
    ▼ (深度優先遍歷 500,000 個 Commit 物件)
    │
Client (Haves: C_480000) ───► [ 尋找最近共同祖先 ]
```
...
```
+-------------------------------------------------------------+
| OID Fanout Table (256 entries for fast prefix lookup)       |
+-------------------------------------------------------------+
| OID Lookup Table (Lexicographically sorted Commit Hashes)   |
+-------------------------------------------------------------+
| Commit Data Chunk (Root Tree OID, Parent 1, Parent 2)       |
+-------------------------------------------------------------+
| Generation Number Chunk (Pre-calculated Topological Depth)  |
+-------------------------------------------------------------+
```
...
```
Commit A (Gen: 1) ──► Commit B (Gen: 2) ──► Commit C (Gen: 3)
                              ▲
Commit X (Gen: 1) ────────────┘
```
