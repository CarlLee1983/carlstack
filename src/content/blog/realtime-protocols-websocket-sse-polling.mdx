...
```text
[Short Polling]
Client ─── HTTP GET ───> Server (無資料，立即回傳空 200 OK)
Client ─── HTTP GET ───> Server (無資料，立即回傳空 200 OK)
Client ─── HTTP GET ───> Server (有新資料，回傳資料 200 OK)

[Long Polling]
Client ─── HTTP GET ───> Server (無資料，掛起連線等待...)
                                  ... 5 秒後產生新訊息 ...
Client <── 200 OK ────── Server (帶回新訊息)
Client ─── HTTP GET ───> Server (立即建立下一輪掛起連線...)
```
