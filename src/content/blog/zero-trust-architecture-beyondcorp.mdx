...
[ 請求發起 (員工設備 / 行動裝置) ]
             │
             ▼
[ 統一上下文感知存取代理 (Access Proxy / Envoy) ]
             │
             ├── 1. 查詢身分 (IdP / Okta) ──> (多因素認證 MFA, OAuth/OIDC)
             ├── 2. 設備信任引擎 (Device Inventory) ──> (TPM 晶片證書, 磁碟加密狀態, EDR 代理)
             └── 3. 動態存取控制引擎 (Policy Engine / ABAC)
                     │
                     ▼ (動態判定：符合安全等級 4 且處於上班時區)
[ 內部微服務 / 資料中心 / SaaS 應用 ] ── (服務間強制執行 mTLS 微隔離)
```
...
```text
[ 訂單服務 (Order Service) ]                       [ 支付服務 (Payment Service) ]
           │                                                    │
     [ Envoy Sidecar ] ─── (雙向 mTLS: 驗證 SPIFFE ID) ───> [ Envoy Sidecar ]
           │                                                    │
     (發送 x509 證書)                                    (驗證僅允許 order-svc 呼叫)
```
