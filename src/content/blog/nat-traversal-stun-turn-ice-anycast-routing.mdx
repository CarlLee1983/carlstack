...
```text
       [ STUN Server ] ── (發現外網映射 IP:Port)
              ▲
              │ (若雙方皆非對稱型 NAT: 直接 P2P 打洞)
   Peer A ◄────────────────────────────────────────► Peer B
              │ (若其中一方或雙方為對稱型 NAT: 降級中繼)
              ▼
       [ TURN Relay Server ] ── (轉發全部流量)
```
...
```text
[Unicast (單播)]
全球各地的 Client ─── (透過不同 IP 或固定跨洋光纖) ───> 集中在某單一機房的 Server

[Anycast (選播)]
全球多個機房共同廣播【完全相同的一個 IP 位址】(e.g., 198.51.100.1)
  - 美國 Client ── (最短 BGP AS-Path) ──> 洛杉磯 Edge Node
  - 歐洲 Client ── (最短 BGP AS-Path) ──> 法蘭克福 Edge Node
  - 亞洲 Client ── (最短 BGP AS-Path) ──> 東京 Edge Node
```
