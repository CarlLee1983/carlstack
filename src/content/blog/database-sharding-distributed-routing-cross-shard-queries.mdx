...
```text
[ 原始單體大庫: 包含 Users, Orders, Products, Payments 全部資料表 ]
                               │
            ┌──────────────────┴──────────────────┐
            ▼                                     ▼
   【垂直拆分 (按業務邊界)】             【水平拆分 (按資料行切分)】
   - User DB (Users 表)                  - Order DB 1 (Orders 1~500 萬)
   - Order DB (Orders 表)                - Order DB 2 (Orders 500~1000 萬)
   - Product DB (Products 表)            - Order DB 3 (Orders 1000~1500 萬)
```
