...
```text
Disk ──(DMA)──► Kernel Page Cache ──(CPU)──► User App Buffer ──(CPU)──► Socket Buffer ──(DMA)──► NIC
```

Kafka 在消費者拉取訊息時，利用了 Linux 核心的 **`sendfile()`** 系統呼叫（即 **Zero-Copy** 技術）：

```text
Kernel Page Cache ──(DMA Direct Copy)──► Network Interface (NIC)
```
