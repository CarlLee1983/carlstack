...
```
[ Function foo() Specification ]
  • Pre:  ptr != null
  • Post: return ptr->value + 1

[ Function bar() Calling foo(x) ]
  • Infer 僅需比對 bar 傳入的 x 是否滿足 foo 的 Precondition
  • 完全無需重新分析 foo 的內部實作細節！
```
...
```
            ...
            ┌────────────────┼────────────────┐
            ▼                ▼                ▼
    [ Template-Based ]  [ Mutation-Based ]  [ History-Based ]
    • Null Guard 範本   • AST 節點條件反轉    • 學習過往人類修復
    • 資源 Try 封裝      • 變數作用域調整      • 統計機器學習模型
            │                │                │
            └────────────────┼────────────────┘
                             ▼
               [ Candidate Patches (5~20 個) ]
```
