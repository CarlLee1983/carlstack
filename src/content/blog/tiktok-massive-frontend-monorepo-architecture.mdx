...
```
  ...
  ┌──────────────┼──────────────┐
  ▼ (受影響)      ▼ (受影響)      ▼ (無關 - 跳過)
[UI 元件庫]    [TikTok Web]   [TikTok Shop] (0% 運算浪費)
```
...
┌──────────────────────────────────────────────────────────┐
│ 主應用基座 (Shell Application) │
│ (共用導航欄、用戶登入態、全域主題、Garfish 核心調度器) │
└────────────────────────────┬─────────────────────────────┘
│
┌───────────────────────┼───────────────────────┐
▼ ▼ ▼
┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐
│ 短影音播放子應用 │ │ 電商直播子應用 │ │ 創作者中心子應用 │
│ (Garfish 沙盒) │ │ (Garfish 沙盒) │ │ (Garfish 沙盒) │
│ • JS 全域變數隔離│ │ • 獨立 Vue 實例 │ │ • 獨立 React 18 │
│ • CSS 樣式作用域 │ │ • 獨立發布部署 │ │ • 動態遠端載入 │
└──────────────────┘ └──────────────────┘ └──────────────────┘
