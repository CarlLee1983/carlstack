...
```text
[ 攻擊場景 ]
攻擊者 (User ID: 999) ───> GET /api/v1/invoices/10086 ───> 後端伺服器
                                                              │
                                  ❌ 漏洞代碼：僅驗證 Token 有效，未校驗租戶歸屬
...
```text
JWT 結構：[ Header (演算法, 類型) ].[ Payload (用戶身份, 過期時間) ].[ Signature (簽名) ]
```
...
```text
[ 密鑰混淆攻擊 (Algorithm Confusion) ]
伺服器原本預期使用 RS256 (非對稱加密：公鑰驗證，私鑰簽名)
攻擊者獲取了伺服器的【公開公鑰 (Public Key)】(通常是公開可得的 PEM 格式)
攻擊者將 Token Header 改為 HS256 (對稱加密：同一個 Secret 簽名與驗證)
攻擊者使用伺服器的【公開公鑰字串】作為 HS256 的 Secret 來生成偽造簽名！
                               │
伺服器執行驗證時，若使用了同一個 verifyKey(publicKey)：
HS256 演算法誤將公鑰字串當作對稱密鑰校驗 ➔ 驗證通過! 攻擊者成功偽造任意身分!
```
