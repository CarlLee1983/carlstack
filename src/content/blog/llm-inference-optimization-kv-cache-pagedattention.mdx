...
為了避免在每次生成新 Token 時重複計算歷史 Prompt 的 Key 和 Value 矩陣，推理引擎會將過去所有層的 K 與 V 張量快取在顯存中，這就是 **KV Cache**。

### 1. KV Cache 顯存佔用數學公式

對於一個標準 Transformer 模型，單一請求單一 Token 的 KV Cache 顯存佔用為：

```text
KV_Size_Per_Token = 2 * n_layers * n_heads * head_dim * precision_bytes
```
...
```text
Request 1 邏輯塊:
  Logical Block 0 (Token 0..15)   ➔ Physical Block 7
  Logical Block 1 (Token 16..31)  ➔ Physical Block 2
  Logical Block 2 (Token 32..47)  ➔ Physical Block 9 (按需即時分配)
```
...
gantt
    ...
    Request A (50 tokens)  :done, 0, 50
    Request B (200 tokens) :active, 0, 200
    Request C (Padding 150):crit, 50, 200
    section Continuous Batching
    Request A (50 tokens)  :done, 0, 50
    Request D (隨即插隊)   :active, 50, 180
    Request B (200 tokens) :active, 0, 200
```
