...
| 場景模組                    | 公開案例數 | 代表性開源或官方專案                                                              | 承擔的判斷環節                                                                                | 外部程式接續的動作                                                             |
| :-------------------------- | :--------- | :-------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------- |
| **01 · GEO 與營銷**         | 2 條       | `usenotra/notra`、`replynodes/jev-web-analyzer`                                   | AI 回答中是否實質提及品牌；官網 Markdown 表達清晰度與價值主張完整性                           | 記錄提及分佈與 URL；生成多維表達診斷報告供人工優化                             |
| **02 · 知識檢索與證據**     | 9 條       | `superagents-lab/jev-search`、`MarissaFamularo/citation-verifier`、`jexp/neo4jev` | 檢索結果語義重排；RAG 證據前置過濾；引用句是否實質支持論斷；知識圖譜下一跳選擇                | 剔除無效檢索；高亮矛盾引文並轉人工；程式碼驗證引文原文存在性                   |
| **03 · 文檔與數據工程**     | 9 條       | `mattn/sqlite-jev`、`AkashPriyadarshii/jev-curate`                                | GDPR 條款多維平行標註；抽取後逐欄位一致性校驗；分類樹逐層走訪；訓練資料品質評分               | 觸發更強模型修正抽取錯誤；過濾 Parquet/JSONL 訓練集；SQL 函數內即時篩選        |
| **04 · 業務分流與風控**     | 7 條       | TypeSafe 理賠 cookbook、Vercel 工單分流、LangGraph 郵件發票路由                   | 車險理賠要件符合度；公司 SIC 行業分類；發票與合同四方交叉驗證；客服承諾兌現審計               | 疑點分流至人工佇列；自動路由至財務或工單系統；未履約承諾發出告警               |
| **05 · Agent 編排與安全**   | 9 條       | LangChain AutoMode、`noelzappy/tripwire`、`AshutoshVJTI/progressgate`             | 工具呼叫前風險評估；生成回答出廠檢驗；Agent 探索是否陷入重複無效迴圈                          | 阻斷高危 Bash 執行；攔截違規生成內容；提示重規劃或強制終止 Agent run           |
| **06 · 瀏覽器與設備自動化** | 7 條       | `browser-use/jev-ultrafast`、`droidrun`、`kitze/unclutter`、HA-Jev                | 依當前已索引 DOM 選擇點擊或導航目標；Android 介面動作選擇；網頁干擾元素判定                   | 外部瀏覽器驅動執行點擊；隱藏干擾 DOM；智慧家居感測器狀態切換                   |
| **07 · 開發與維運**         | 7 條       | `sufianetaouil/every`、`frostney/clean-code-review`、`Vicente-MD/jev-resilience`  | 依自然語言找函數；PR 差異多維品質檢查；Git Commit 說明與實際改動一致性；HTTP 200 業務錯誤識別 | 排列候選程式碼片段；生成分類 review 反饋；標記未記錄的代碼變更；觸發告警與重試 |
| **08 · 內容與媒體審核**     | 7 條       | Every 21 項寫作模式檢查、`valentynkit/jev-skip`、`santos-sanz/jev-audio-beeper`   | 字幕軌贊助廣告區間識別；影片轉錄逐句審查；語音詞級時間戳髒話檢測；文章 AI 味診斷              | 播放器跳過片頭贊助；ffmpeg 精確消音插入提示音；Discord 違規發言標記            |
| **09 · 交互與能力評測**     | 3 條       | Doom 遊戲動作循環、LLM Chess 合法步選擇、`nekuda-ai/WindTunnel`                   | 結構化遊戲狀態下一步選擇；國際象棋合法棋步選優；WebMCP 結構化工具呼叫基準對抗                 | 遊戲引擎執行動作；棋盤程式驗證走步有效性；比較 DOM 模式與工具模式的吞吐量      |
