...
```text
[ 歷史訊息 ] ───────> [ 某個時刻：設備私鑰洩漏 / 被攻破 ] ───────> [ 未來訊息 ]
      │                                                                  │
      ▼                                                                  ▼
前向保密性 (Forward Secrecy, FS)                               後向安全性 (Post-Compromise Security, PCS)
過去的歷史訊息依然無法被解密！                                 協議自動自我修復，未來的訊息重新恢復安全！
```
...
Signal Protocol 之所以成為業界公認的安全天花板，核心在於兩大演算法：
...
```text
               [ 根棘輪 (Root KDF Ratchet) ]
                     │ (隨著雙方 DH 交換持續旋轉)
        ┌────────────┴────────────┐
        ▼                         ▼
[ 發送鏈棘輪 (Sending Chain) ]   [ 接收鏈棘輪 (Receiving Chain) ]
  ├── 訊息 1 密鑰 ➔ [立即銷毀]     ├── 訊息 1 解密 ➔ [立即銷毀]
  ├── 訊息 2 密鑰 ➔ [立即銷毀]     └── 訊息 2 解密 ➔ [立即銷毀]
  └── ... (每條訊息使用獨立 Message Key)
```
