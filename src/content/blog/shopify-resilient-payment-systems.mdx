...
同步 HTTP 請求鏈條越長，系統整體可用性（Availability）就越低。若結帳 API 同步等待信用卡授權、庫存扣減、發票生成與 Email 通知，任何一個微服務延遲都會導致使用者結帳失敗。
...
```
[客戶端/閘道 Timeout: 5.0s] ──► [內部支付服務 Timeout: 3.5s] ──► [第三方銀行 API Timeout: 2.0s]
```
