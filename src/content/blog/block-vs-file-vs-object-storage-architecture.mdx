...
[ Application (MySQL / PostgreSQL) ]
              │ (Direct I/O / Page Cache)
              ▼
[ OS File System (ext4 / XFS) ]
              │ (Block I/O Requests: LBA 0x004A1F)
              ▼
[ Block Device (AWS EBS / SAN / NVMe-oF) ]
```
...
```text
1. 是否需要資料庫等級的亞毫秒事務隨機讀寫？
   ├── 是 ──► 【區塊儲存】 (AWS EBS io2 / gp3, NVMe SSD)
   └── 否 ──► 進入第 2 步

2. 是否需要現有程式碼以 POSIX 檔案路徑與多節點並行掛載共享？
   ├── 是 ──► 【檔案儲存】 (AWS EFS, GCP Filestore, NFS)
   └── 否 ──► 進入第 3 步

3. 是否為巨量非結構化靜態資料、備份或大數據湖倉？
   └── 是 ──► 【物件儲存】 (AWS S3, Cloudflare R2, MinIO)
```
