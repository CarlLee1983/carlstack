...
describe("OrderService", () => {
  it("應成功建立訂單", async () => {
    ...
    jest.spyOn(StripeClient.prototype, "charge").mockImplementation(mockStripe.charges.create);
...
export interface PaymentGatewayPort {
  authorize(token: string, amount: number): Promise<Result<string, PaymentError>>;
...
export class InMemoryPaymentFake implements PaymentGatewayPort {
  private transactions = new Map<string, { amount: number; status: TransactionStatus }>();
  ...
  async authorize(token: string, amount: number): Promise<Result<string, PaymentError>> {
    if (this.nextError) {
...
describe("OrderModule 訂單結帳整合", () => {
  it("扣款成功時應將訂單標記為已支付並扣抵庫存", async () => {
    ...
    const result = await orderModule.checkout({ cartId: "cart_99", token: "tok_valid" });

    expect(result.isOk()).toBe(true);
    // 斷言系統的真實最終狀態，而非內部呼叫次數
    expect(await fakePayment.getTransactionStatus(result.value.transactionId)).toBe("CAPTURED");
  });
